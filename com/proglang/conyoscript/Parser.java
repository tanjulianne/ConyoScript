...
import static com.proglang.conyoscript.TokenType.*;
...
//< Statements and State parse-statement
  private Stmt doStatement() {
    ...
    consume(SEMICOLON, "Expect ';' after 'while'");
...
//> Control Flow if-statement
  private Stmt ifStatement() {
    ...
    if (condition instanceof Expr.Assign) {
      consume(EQUAL, "Invalid condition");
    }
  ...
  private Stmt printStatement() {
    ...
    if (!(value instanceof Expr.Literal)) {
      if (!(value instanceof Expr.Variable)) {
        consume(STRING, "Expect string or variable after print.");
      }
    }
...
//> Statements and State parse-print-statement
    private Stmt scanStatement() {
        consume(LEFT_PAREN, "Expect '(' after scan.");
        Token name = consume(IDENTIFIER, "Expect variable name.");
        consume(RIGHT_PAREN, "Expect ')' after variable.");
        consume(SEMICOLON, "Expect ';' after parenthesis.");
...
//< Statements and State parse-identifier
    ...
    if (match(SCAN)) {
      return call(new Expr.Variable(previous()));
    }
//> primary-error
